...
function generateTableView(total_steps,i1_size) {
    var table_rows = "<tr><th>Step Name</th><th>Static</th><th>Editor types</th><th>No. of Editors object</th><th>No. of Dropdowns object</th><th>No. of Tries</th><th>Has Step Label</th><th>Separate AP for Dropdown</th><th>Total no. of statements</th><th>No. of passive statements</th><th>Position of editor</th><th>Position of table (Comma separated list)</th></tr>";
    var tool_select=` 
  ...
  `;
    for(let i = 1; i<=total_steps; i++) {   
        ...
        `;
        if (i<=i1_size){
        table_rows += "<tr><td>"+totalModuleList[i-1]+"</td><td><input style='display:none;' type='checkbox' id='checkbox-"+i+"' name='static' value='static' onchange='toggleSelection("+i+")'></td><td><select id='tools-"+i+"' name='tool-"+i+"'>"+tool_select+"</select></td><td><input name='editor-"+i+"' id='editor-"+i+"' size='1' value='1' /></td><td><input name='ddm-"+i+"' size='1' id='ddm-"+i+"' value='0'/></td><td><input name='tries-"+i+"' id='tries-"+i+"' type=number size='1' min='1' max='3' value='3' /></td><td></td><td><input type='checkbox' id='separate-"+i+"' name='separate-"+i+"' /></td><td><input name='text-"+i+"' size='1' id='text-"+i+"' value='0'/></td><td><input name='passivetext-"+i+"' size='1' id='passivetext-"+i+"' value='0'/></td><td><input name='edittext-"+i+"' size='1'  id='edittext-"+i+"' value='0'/></td><td><input name='table-"+i+"' id='table-"+i+"' value='0'/></td></tr>"
        }
        else{
...
function parseTableResponse(i1_size) {
    ...
    for(let i=1; i<=totalModuleList.length; i++) {
        ...
        if(i<=i1_size){
            ...
            module_data = {module_name:totalModuleList[i-1],isStatic:bStatic,tool_type:tools_name,editor_count:editors_count,ddm_count:ddms_count,isSeparateApNeeded:bSeparate,total_tries:noTries,textstatement:text_statement,passivestatement:passive_statement,edit_statement:edit_statement,table:array_table};
        }
        else{
            hasStepId = "gsstep-"+i;
            steplabel = document.getElementById(hasStepId).checked;
            module_data = {module_name:totalModuleList[i-1],isStatic:bStatic,tool_type:tools_name,editor_count:editors_count,ddm_count:ddms_count,isSeparateApNeeded:bSeparate,hasLabel:steplabel,textstatement:text_statement,passivestatement:passive_statement,edit_statement:edit_statement,table:array_table};
        }
